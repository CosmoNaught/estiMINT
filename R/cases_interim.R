...
build_case_models <- function(db_path,
  ...
  if (plotting && !dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE, showWarnings = FALSE)

  ## ------------------------------------------------------------------------
  con <- dbConnect(duckdb::duckdb(), db_path, read_only = TRUE)
  on.exit(dbDisconnect(con))

  ts  <- get_timestep_window(con, "simulation_results")   # <- unchanged
...
feature_cols <- c(
  ...
  utils::write.csv(metrics_df,
                   file.path(model_dir, "case_model_metrics.csv"),
                   row.names = FALSE)
